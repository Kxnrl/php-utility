...
function vdf_decode(string $file) : array {

    $text = file_get_contents($file);

    if(!is_string($text)) {
        trigger_error("vdf_decode expects parameter 1 to be a string, " . gettype($text) . " given." . PHP_EOL . 'file: ' . $file, E_USER_NOTICE);
        return NULL;
    }
    ...
    for($i = 0; $i < $j; $i++) {
        ...
        if($expect_bracket) {
            trigger_error("vdf_decode: invalid syntax, expected a '}' on line " . ($i+1) . PHP_EOL . 'file: ' . $file, E_USER_NOTICE);
            return Null;
        }
        ...
        while(True) {
            ...
            if(!$m) {
                trigger_error("vdf_decode: invalid syntax on line " . ($i+1) . PHP_EOL . 'file: ' . $file, E_USER_NOTICE);
                return NULL;
            }
    ...
    if(count($stack) !== 1)  {
        trigger_error("vdf_decode: open parentheses somewhere" . PHP_EOL . 'file: ' . $file, E_USER_NOTICE);
        return NULL;
    }
